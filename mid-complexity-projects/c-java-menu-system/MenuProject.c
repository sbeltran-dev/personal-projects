...
int main () {
    ...
    do {
        ...
        if (scanf("%d", &option) != 1) { // * In case the user enters a differente data type.
        ...
        if (option == 0) { // * If the input is '0'.
            printf("\nClosing system... Thank you for using the programm :)\n");
        } else if (option >= 1 && option <= 5) { // * If the option is available in the menu.
        ...
        if (option != 0) { // * Pauses the programm until the users presses 'ENTER'.
...
// ** FUNCTIONS TO EXECUTE THE JAVA PROGRAMM.
void execute2ndOption() {
...
void showMenu() {
    ...
    printf("%s5. While cicle for printing numbers pattern.%s\n", COLOR_GREEN, COLOR_RESET);
    printf("%s0. Leave the system%s\n", COLOR_CYAN, COLOR_RESET);
}

// ** WE USE THIS FUNCTION TO DELETE THE '\n' TO NOT INTERFERE IN THE PROGRAMM.
void cleanBuffer() {
    int c;
    while ((c = getchar()) != '\n' && c != EOF);
}
